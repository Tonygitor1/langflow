...
const ModelProvidersContent = ({
  ...
  const { syncedSelectedProvider } = useProviderConfiguration({
    selectedProvider,
  });

  // Expose a no-op flush to the parent (no pending changes in read-only mode)
  useEffect(() => {
    if (onFlushRef) {
      onFlushRef.current = async () => {};
    }
    return () => {
      if (onFlushRef) {
      ...
      >
        <div className="px-4 pt-3 pb-1">
          <p className="text-xs text-muted-foreground">
            Models are managed centrally via LiteLLM. Contact your platform
            admin to add or remove models.
          </p>
        </div>
