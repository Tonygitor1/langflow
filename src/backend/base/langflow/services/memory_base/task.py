...
async def _pg_advisory_lock(key: int) -> None:
    """Acquire a PostgreSQL session-level advisory lock with retry and timeout.

    Raises asyncio.TimeoutError if the lock cannot be acquired within
    the configured timeout (max_ingestion_timeout_secs).
    """
...
async def _pg_advisory_unlock(key: int) -> None:
...
async def _acquire_session_lock(memory_base_id: uuid.UUID, session_id: str) -> int | asyncio.Lock:
    """Acquire the distributed ingestion lock. Returns the key (PG) or Lock (SQLite).

    Raises asyncio.TimeoutError if the lock cannot be acquired in time.
    """
    timeout = get_settings_service().settings.max_ingestion_timeout_secs
    if await _is_postgres():
        key = _compute_advisory_key(memory_base_id, session_id)
        await _pg_advisory_lock(key)
        return key
...
async def _release_session_lock(lock_handle: int | asyncio.Lock) -> None:
    """Release the distributed ingestion lock."""
    if isinstance(lock_handle, int):
        await _pg_advisory_unlock(lock_handle)
    else:
        lock_handle.release()
...
async def ingest_memory_task(*, request: IngestionRequest) -> dict:
    ...
    try:
        lock_handle = await _acquire_session_lock(memory_base_id, session_id)
    ...
    try:
        ...
        if not messages:
            await logger.ainfo("MemoryBase %s / session %s: no pending messages, skipping.", memory_base_id, hashed_sid)
            return {"message": "No pending messages", "ingested": 0}

        # ---- 2. Build documents from messages ----
        documents = build_documents_from_messages(messages, session_id=session_id, flow_id=str(flow_id))
        ...
        try:
            ...
            if written == len(documents):
                ...
                # Wrapped in to_thread to avoid blocking the event loop with sync I/O.
        ...
        await _release_session_lock(lock_handle)
